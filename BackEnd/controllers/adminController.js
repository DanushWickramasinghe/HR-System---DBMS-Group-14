...
const postAdminLogin = async (req, res) => {
  ...
  let sql1 = `SELECT * FROM Employee left outer join EmpType on Employee.type = EmpType.ID left outer join User on Employee.user_Id = User.ID WHERE EmpType.type = 'Admin' and user.username = ${con.escape(
    username
  )} LIMIT 1`;
  ...
  con.query(sql1, async function (err, result) {
    if (err) {
      ...
      if (result.length < 1) {
        ...
        const auth_password = await enc.checkEncryptedCredential(
          password,
          hashed_password
        );
